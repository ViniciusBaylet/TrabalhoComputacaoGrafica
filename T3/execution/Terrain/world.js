...
export class World extends THREE.Group {
  ...
  generateTerrain() {
    ...
    const waterLevel = Math.floor(this.size.height * 0.2) / 4;
        ...
        for (let y = 0; y <= this.size.height; y++) {
          for (let y = 0; y <= this.size.height; y++) {
            if (y < height - 4) {
              ...
              } else if (surfaceChance < 0.0005) {
                this.setBlockId(x, y, z, blocks.sapling.id);
              } else {
                this.setBlockId(x, y, z, blocks.grass.id);
              }
            } else if (y <= waterLevel) {
              // Preencher com água até o nível do mar
